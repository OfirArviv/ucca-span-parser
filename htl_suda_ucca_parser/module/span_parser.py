...
from htl_suda_ucca_parser.convert import InternalParseNode, LeafParseNode
...
class Topdown_Span_Parser(SpanParser):
    def __init__(self, vocab: Vocabulary, span_dim, dropout=0.2):
        ...
        self.vocab = vocab
        num_layers = 2
        self.label_ffn = FeedForward(span_dim, num_layers, [200, vocab.get_vocab_size("labels")], torch.relu)
        self.split_ffn = FeedForward(span_dim, num_layers, [200, 1], torch.relu, dropout)

    def forward(self, spans):
        label_score = self.label_ffn(spans)
        split_score = self.split_ffn(spans).squeeze(-1)
    ...
    def helper(self, label_scores, split_scores, sen_len, left, right, gold=None):
        ...
        if self.training and gold is not None:
            ...
            oracle_label_index = self.vocab.get_token_index(str(oracle_label), "labels")
            label_score = self.augment(label_score, oracle_label_index)
            oracle_label_score = label_score[oracle_label_index]

        # Root label, so must have a valid label
        if right - left == sen_len:
            root_labels_indexes = [key for key, value in self.vocab.get_index_to_token_vocabulary("labels").items()
                                   if 'ROOT' in str(value)]
            not_root_labels_indexes = [i for i in range(len(label_score)) if i not in root_labels_indexes]
            label_score[not_root_labels_indexes] = float("-inf")

        argmax_label_score, argmax_label_index = torch.max(label_score, dim=0)
        argmax_label = make_tuple(self.vocab.get_token_from_index(int(argmax_label_index), "labels"))
